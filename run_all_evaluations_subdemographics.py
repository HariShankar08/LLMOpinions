...
def run_evaluation(region, script_name, log_suffix=""):
    ...
    # Determine demographic vars list for region
    if region == 'IND':
        demographic_vars = IND_DEMOGRAPHIC_VARS
    elif region == 'EA':
        demographic_vars = EA_DEMOGRAPHIC_VARS
    else:
        demographic_vars = SEA_DEMOGRAPHIC_VARS
    ...
    for json_file in json_files:
        ...
        # Load JSON to fetch options per demographic var
        try:
            with open(json_file, 'r', encoding='utf-8') as jf:
                questions_map = json.load(jf)
        except Exception as e:
            log_message(f"Warning: Could not read questions JSON '{json_file}': {e}", Colors.YELLOW)
            continue
